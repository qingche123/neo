...
{
    [TestClass()]
    public class SM3ManagedTests
    {
        private string[,] sm3TestIV;
        private static ThreadLocal<SM3Managed> _sm3 = new ThreadLocal<SM3Managed>(() => new SM3Managed());

        public bool ArrayCmp(byte[] b1, byte[] b2)
        {
        ...
        [TestMethod()]
        public void SM3ManagedTest()
        {
            sm3TestIV = new string[31, 2]{
            ...
            for (int i = 0; i < 31; i++)
            {
                byte[] sm3Hash = new byte[64];
                byte[] bHashIV = sm3TestIV[i, 0].HexToBytes();
                byte[] bTextIV = System.Text.Encoding.ASCII.GetBytes(sm3TestIV[i, 1]);
                
                sm3Hash = _sm3.Value.ComputeHash(bTextIV);
                if (!ArrayCmp(sm3Hash, bHashIV))
                {
                    Assert.Fail();
                }
            }
